...
static int getInt(int* pResultado)
{
	...
	if(pResultado != NULL && utn_myGets(buffer, sizeof(buffer)) == 0 && utn_esNumerico(buffer, sizeof(buffer)) == TRUE && buffer[0] != ' ' && buffer[0] != '\0')
	{
...
static int getFloat(float* pResultado)
{
	...
	if(pResultado != NULL && utn_myGets(buffer, sizeof(buffer)) == 0 && utn_esFloat(buffer, sizeof(buffer)) == TRUE && buffer[0] != ' ' && buffer[0] != '\0')
	{
...
int utn_getAlfaNumerico(char* input, char* mensaje, char* mensajeError, int longitud, int reintentos)
{
    ...
    if(input != NULL && mensaje != NULL && mensajeError != NULL && longitud > 0 && reintentos >= 0)
    {
        do
        {
            ...
            if((utn_myGets(bufferString, longitud) == 0) && (utn_esAlfaNumerico(bufferString, sizeof(bufferString)) != FALSE) && bufferString[0] != ' ' && bufferString[0] != '\0')
            {
                strncpy(input, bufferString, longitud);
                retorno = 0;
                break;
