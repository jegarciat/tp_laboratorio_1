...
int utn_getString(char* input, char* mensaje, char* mensajeError, int longitud, int reintentos)
{
    ...
    if(input != NULL && mensaje != NULL && mensajeError != NULL && longitud > 0 && reintentos >= 0)
    {
        do
        {
            ...
            if((utn_myGets(bufferString, longitud) == 0) && (utn_SoloLetras(bufferString) != FALSE) && bufferString[0] != ' ')
            {
...
static int getInt(int* pResultado)
{
	...
	if(pResultado != NULL && utn_myGets(buffer, sizeof(buffer)) == 0 && utn_esNumerico(buffer, sizeof(buffer)) == TRUE)
	{
...
static int getFloat(float* pResultado)
{
	...
	if(pResultado != NULL && utn_myGets(buffer, sizeof(buffer)) == 0 && utn_esFloat(buffer, sizeof(buffer)) == TRUE)
	{
...
int utn_esAlfaNumerico(char* str, int limite)
{
	int i;
	int retorno;
	retorno = -1;
	...
	if(str != NULL && limite > 0)
	{
		...
		{
			if((str[0] != ' ') && (str[i] < 'a' || str[i] > 'z') && (str[i] < 'A' || str[i] > 'Z') && (str[i] < '0' || str[i] > '9'))
			{
				retorno = FALSE;
				break;
			}
...
int utn_getAlfaNumerico(char* input, char* mensaje, char* mensajeError, int longitud, int reintentos)
{
    ...
    if(input != NULL && mensaje != NULL && mensajeError != NULL && longitud > 0 && reintentos >= 0)
    {
        do
        {
            ...
            if((utn_myGets(bufferString, longitud) == 0) && (utn_esAlfaNumerico(bufferString, sizeof(bufferString)) != FALSE) && bufferString[0] != ' ')
            {
                strncpy(input, bufferString, longitud);
                retorno = 0;
                break;
